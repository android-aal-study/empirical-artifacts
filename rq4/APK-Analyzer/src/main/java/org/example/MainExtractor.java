...
public class MainExtractor extends Extractor {
    ...
    public MainExtractor(String apkPath, String androidJar, String veridexPath) {
        super(apkPath, androidJar, veridexPath);
    }
    ...
    public static void main(String[] args) {
        if(args.length != 4) {
            ...
            System.err.println("    java -jar executable.jar <android.jar-directory> <apk-path> <output-directory> <veridex-directory>");
            System.exit(-1);
        }
        ...
        String veridexPath = args[3];

        ExecutorService executor = Executors.newSingleThreadExecutor();
        Future<String> future = executor.submit(() -> {
            try {
                System.out.println("Processing " + apkPath);
                MainExtractor mainExtractor = new MainExtractor(apkPath, androidJar, veridexPath); //analyze apk
